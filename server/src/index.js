...
// ==== FIX HOSTINGER PATHS (não depende do cwd) ====
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename); // .../server/src
const ROOT_DIR = path.resolve(__dirname, "..", ".."); // raiz do repo
...
// aceita CORS_ORIGIN como:
// - "https://docarmo.store"
// - "https://docarmo.store,https://www.docarmo.store"
const CORS_ORIGIN_RAW = process.env.CORS_ORIGIN || "http://localhost:8080";
const CORS_ORIGIN = CORS_ORIGIN_RAW.includes(",")
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      const safe =
        slugify(file.originalname.replace(/\.[^/.]+$/, "")) || "file";
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!parsed.ok)
    return res.status(400).json({ error: "invalid_body", details: parsed.details });
...
app.post("/api/checkout", async (req, res) => {
  ...
  try {
    ...
    const preferenceBody = {
      ...
      back_urls: {
        success: `${base}/pagamento/sucesso?o=${order.orderNumber}`,
        pending: `${base}/pagamento/pendente?o=${order.orderNumber}`,
        failure: `${base}/pagamento/erro?o=${order.orderNumber}`,
      },
    ...
    return res.json({ orderNumber: order.orderNumber, checkoutUrl: initPoint });
  } catch (e) {
